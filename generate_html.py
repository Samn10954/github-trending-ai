...
"""
生成静态页面：
- 支持 daily / weekly / monthly 三套真实数据
- 每个 period 生成独立页面
- 支持项目级趋势图、tag filter、排序、收藏
"""
...
TAG_META = [
    ("all", "全部"), ("agent", "Agent"), ("llm", "LLM"), ("robotics", "机器人/具身"),
    ("vision", "视觉/CV"), ("multimodal", "多模态"), ("speech", "语音/TTS"),
    ("rag", "RAG"), ("training", "训练/推理"),
]
...
def load_period_records(period: str) -> list[dict[str, Any]]:
    period_dir = DATA_ROOT / period
    if not period_dir.exists():
        return []
    records = []
    for path in sorted(period_dir.glob("*.json"), reverse=True):
...
def build_project_trends(records: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    trends: dict[str, list[dict[str, Any]]] = {}
    for record in sorted(records, key=lambda r: r["date"]):
        for p in record.get("projects", []):
            trends.setdefault(p["name"], []).append({
                "date": record["date"],
                "stars_period": p.get("stars_period", 0),
                "stars": p.get("stars", 0),
            })
    return trends


def build_date_nav(records: list[dict[str, Any]], current_date: str, current_period: str) -> str:
    items = []
    for r in records:
        active = " active" if r["date"] == current_date else ""
        items.append(f'<a href="./{current_period}-{r["date"]}.html" class="date-btn{active}">{format_date_label(r["date"])}' + '</a>')
    return "\n".join(items)


def build_period_buttons(current_period: str) -> str:
    return "\n".join(
        f'<a href="./{key}.html" class="tag-btn period-btn{" active" if key == current_period else ""}">{label}</a>'
...
def build_archive(records: list[dict[str, Any]], current_period: str) -> str:
    ...
    for r in records:
        ...
        parts.append(
            f'<a class="archive-item" href="./{current_period}-{r["date"]}.html"><span>{format_date_label(r["date"])} / {r["date"]}</span><span>{count} 个项目</span></a>'
        )
    return "\n".join(parts)


def render_page(record: dict[str, Any], records: list[dict[str, Any]], period: str, project_trends: dict[str, list[dict[str, Any]]], *, is_period_home: bool = False) -> str:
    projects = record.get("projects", [])
    current_date = record["date"]
    total_stars = sum(p.get("stars_period", 0) for p in projects)
    title = f"GitHub Trending AI - {period} - {current_date}"
    projects_json = json.dumps(projects, ensure_ascii=False)
    trends_json = json.dumps(project_trends, ensure_ascii=False)
    home_href = f"./{period}.html"
...
<head>
  ...
  <style>
    * {{ box-sizing:border-box; margin:0; padding:0; }}
    ...
    .date-btn,.tag-btn,.sort-select,.trend-btn,.fav-btn {{ border:1px solid #30363d; background:#21262d; color:#58a6ff; text-decoration:none; padding:8px 14px; border-radius:8px; }}
    .date-btn.active,.tag-btn.active,.trend-btn.active,.fav-btn.active {{ background:#238636; border-color:#238636; color:#fff; }}
    ...
    .chart-canvas {{ width:100%; height:240px; display:block; background:#0d1117; border:1px solid #30363d; border-radius:8px; }}
...
<body>
  <div class="container">
    <header>
      ...
      <p class="subtitle">{period} 榜单 · 当前日期 {current_date}</p>
    </header>

    <div class="date-nav">{build_date_nav(records, current_date, period)}</div>
    ...
    <section class="chart-section" style="margin-bottom:20px;">
      <div class="section-title">项目趋势图</div>
      <canvas id="trendChart" class="chart-canvas" width="1000" height="240"></canvas>
      <div style="margin-top:10px;color:#8b949e;font-size:.9rem;" id="trendTitle">点击项目卡片中的“查看趋势”查看该项目在历史日期中的变化</div>
    </section>

    <div class="content-grid">
      <div class="project-list" id="projectList"></div>
      <aside class="archive-section"><div class="section-title">历史日期</div><div class="archive-list">{build_archive(records, period)}</div></aside>
    </div>
  </div>

  <script id="projects-data" type="application/json">{projects_json}</script>
  <script id="trends-data" type="application/json">{trends_json}</script>
  <script>
    const projects = JSON.parse(document.getElementById('projects-data').textContent);
    const projectTrends = JSON.parse(document.getElementById('trends-data').textContent);
    ...
    function drawTrendChart(projectName) {{
      ...
      if (!projectName || !projectTrends[projectName] || !projectTrends[projectName].length) {{
        title.textContent = '点击项目卡片中的“查看趋势”查看该项目在历史日期中的变化';
        return;
      }}
    ...
    function renderProjects() {{
      ...
      listEl.innerHTML = sorted.map(p => {{
        ...
        const trendActive = activeTrendProject === p.name;
        ...
        return '<div class="project-card">'
          ...
          + '<button class="fav-btn ' + (fav ? 'active' : '') + '" data-name="' + escapeHtml(p.name) + '">' + (fav ? '★ 已收藏' : '☆ 收藏') + '</button>'
          + '<button class="trend-btn ' + (trendActive ? 'active' : '') + '" data-name="' + escapeHtml(p.name) + '">查看趋势</button>'
          + '</div></div>';
      }}).join('');
      document.querySelectorAll('.fav-btn').forEach(btn => btn.addEventListener('click', () => toggleFavorite(btn.dataset.name)));
      document.querySelectorAll('.trend-btn').forEach(btn => btn.addEventListener('click', () => {{ activeTrendProject = btn.dataset.name; drawTrendChart(activeTrendProject); renderProjects(); }}));
    }}
...
def main() -> None:
    period_records = {period: load_period_records(period) for period in PERIODS}
    latest_home_written = False
    for period, records in period_records.items():
        if not records:
            continue
        trends = build_project_trends(records)
        latest = records[0]
        (BASE_DIR / f"{period}.html").write_text(render_page(latest, records, period, trends, is_period_home=True), encoding="utf-8")
        for record in records:
            (BASE_DIR / f"{period}-{record['date']}.html").write_text(render_page(record, records, period, trends), encoding="utf-8")
        if period == "weekly":
            INDEX_FILE.write_text(render_page(latest, records, period, trends, is_period_home=True), encoding="utf-8")
            latest_home_written = True
    if not latest_home_written:
        INDEX_FILE.write_text("<html><body><h1>No data yet</h1></body></html>", encoding="utf-8")
    print("Generated multi-period pages")
